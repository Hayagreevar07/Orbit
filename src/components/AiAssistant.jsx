...
export default function AiAssistant({ onAction, onNotify, onAgentUpdate, context = {} }) {
  ...
  return <><Tooltip title="Orbit AI command layer"><Button variant="contained" startIcon={<AutoAwesomeRoundedIcon />} onClick={() => setOpen(true)} className="ai-primary-button">Orbit AI</Button></Tooltip><Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm" className="ai-dialog"><DialogTitle><Stack direction="row" alignItems="center" justifyContent="space-between"><Box><Typography className="card-kicker">Orbit AI command layer</Typography><Typography variant="h2">What should Orbit do?</Typography></Box><IconButton onClick={() => setOpen(false)}><CloseRoundedIcon /></IconButton></Stack></DialogTitle><DialogContent><Typography className="muted ai-intro">Orbit sees your current workspace and turns natural language into a real, reviewable operation.</Typography>{!aiConfigured && <Alert severity="info" className="ai-alert">Add `VITE_OPENROUTER_API_KEY` to enable Mistral through OpenRouter.</Alert>}<form onSubmit={ask}><TextField autoFocus fullWidth multiline minRows={3} placeholder="e.g. Add a 45 minute study block for distributed systems at 2pm" value={command} onChange={(event) => setCommand(event.target.value)} disabled={!aiConfigured} /><Button type="submit" variant="contained" endIcon={loading ? <CircularProgress size={16} /> : <SendRoundedIcon />} disabled={!aiConfigured || loading || !command.trim()} className="ai-submit">Run Orbit plan</Button></form>{error && <Alert severity="error" className="ai-alert">{error}</Alert>}{proposal && <Box className="ai-proposal"><Typography className="card-kicker">Orbit proposes</Typography><Typography className="proposal-message">{proposal.message}</Typography><Typography className="muted">{proposal.action?.type === 'none' ? 'No data will change.' : `${proposal.action?.type?.replace('_', ' ')}${proposal.action?.title ? `: ${proposal.action.title}` : ''}`}</Typography></Box>}</DialogContent><DialogActions>{proposal?.action?.type !== 'none' && proposal ? <><Button onClick={() => setProposal(null)}>Revise</Button><Button variant="contained" onClick={approve}>Approve action</Button></> : <Button onClick={() => setOpen(false)}>Close</Button>}</DialogActions></Dialog></>;
}
